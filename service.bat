...
echo      11. Run Tests
echo      12. Autotune Strategy  (Test & Install Best)
echo      13. Edit User Lists    (Console Editor)
echo      14. Configure DNS/DoH  (Secure DNS)
echo      15. Optimize Browsers  (Disable QUIC/Kyber)
echo      16. Open GUI Manager   (Premium Interface)
echo   ----------------------------------------
echo      0. Exit
echo.
...
if "%menu_choice%"=="12" goto service_autotune
if "%menu_choice%"=="13" goto edit_user_lists
if "%menu_choice%"=="14" goto configure_dns
if "%menu_choice%"=="15" goto optimize_browsers
if "%menu_choice%"=="16" goto open_gui
...
echo Downloading hosts from repository...
...
if not exist "%tempFile%" (
    call :PrintRed "Failed to download hosts file."
    pause
    goto menu
)

echo Backing up and updating hosts file...
powershell -NoProfile -Command ^
    "$hostsPath = '%hostsFile%';" ^
    "$tempPath = '%tempFile%';" ^
    "Copy-Item $hostsPath \"$hostsPath.bak\" -Force;" ^
    "$hostsContent = Get-Content $hostsPath -Raw;" ^
    "$newHosts = Get-Content $tempPath -Raw;" ^
    "$startMarker = '# === ZAPRET HOSTS START ===';" ^
    "$endMarker = '# === ZAPRET HOSTS END ===';" ^
    "$pattern = '(?s)' + [regex]::Escape($startMarker) + '.*?' + [regex]::Escape($endMarker);" ^
    "$block = \"$startMarker`r`n$newHosts`r`n$endMarker\";" ^
    "if ($hostsContent -match $pattern) { $hostsContent = $hostsContent -replace $pattern, $block } else { $hostsContent = $hostsContent.TrimEnd() + \"`r`n`r`n\" + $block + \"`r`n\" };" ^
    "[IO.File]::WriteAllText($hostsPath, $hostsContent, [System.Text.Encoding]::UTF8);"

if exist "%tempFile%" del /f /q "%tempFile%"
call :PrintGreen "Hosts file successfully updated!"
pause
goto menu
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0utils\test zapret.ps1" -TestType standard -Mode all -Silent
set "bestStrategyFile=%~dp0utils\best_strategy.tmp"
if not exist "%bestStrategyFile%" (
    call :PrintRed "Autotune failed to determine best strategy."
    pause
    goto menu
)
set /p selectedFile=<"%bestStrategyFile%"
del /f /q "%bestStrategyFile%"
if not defined selectedFile (
    call :PrintRed "Autotune output is empty."
    pause
    goto menu
)
echo Best strategy selected: %selectedFile%
set "selectedFile=%selectedFile%.bat"
